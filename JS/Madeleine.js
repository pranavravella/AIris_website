(function () {
    ...
    Madeleine = function (options) {
        ...
        var OBJECT_BACKGROUND = "FFF";
        var OBJECT_COLOR = "A0A0A0";
        ...
        var VIEWER_HEIGHT = "800";
        var VIEWER_WIDTH = "800";

        var USER_ROTATE_SENSITIVITY = 0.01;
        ...
        Madeleine.prototype.draw = function () {
            ...
            switch (this.type) {
                case "upload":
                    this.getDataFromBlob(this.data, queued);
                    break;
                case "file":
                    this.getDataFromUrl(this.data, queued);
                    break;
                default:
                    break;
            }
        ...
        Madeleine.prototype.getDataFromUrl = function (url, queuedWork, type) {
            ...
            var getArrayBuffer = function () {
                ...
                arrbuf.onerror = function (e) {
                    console.log("MADELEINE[ERR] Ajax failed.")
                };
        ...
        Madeleine.prototype.createViewer = function () {
            if (!this.options.viewer.create) this.container.appendChild(this.__renderer.domElement);
                ...
                info.innerHTML = this.__info.name.split("path=")[1];
        ...
        Madeleine.prototype.adaptViewerTheme = function () {
            ...
            switch (theme) {
                case "dark":
                    this.__viewer.style.background = "#FFF";
                    this.options.objectColor = "A0A0A0";
                    break;
                case "lime":
                    this.__viewer.style.background = "#FFF";
                    this.options.objectColor = "A0A0A0";
            ...
            worker.onmessage = function (event) {
                ...
                switch (result.type) {
                    case "convert-progress":
                        ...
                        break;
                    case "convert":
                        if (CONVERT_TO_BINARY) scope.run(scope.relPath + "lib/MadeleineConverter.js", result.data, function (result) {
                            scope.__converted = result
                        });
                        break;
                    case "progress":
        ...
        Madeleine.prototype.render = function () {
            this.__rotating && this.rotateObjectZ(-0.5);
            ...
            if (options.dropzone) {
                ...
                var fileDropHandler = function (e) {
                    ...
                    for (i = 0; i < files.length; i++) {
                        ...
                        var _options = window.Lily.extend({}, options, {
                            type: "upload",
                            data: files[i]
                        });
                ...
                if (files.length) {
                    for (i = 0; i < files.length; i++) {
                        ...
                        if (files[i].type != 'stl' && files[i].name.indexOf("stl") == -1) {
                            if (options.callbackerror) options.callbackerror({
                                code: 1001,
                                msg: "Invalid File"
                            });
